...
function getPromptUrl(baseURL: string, url: string, content: string) {
    const prompt = `You are an expert Botany tutor reviewing study material from "Nahean's Study Hub".

The current document is located at: ${url}

Based on the provided content, help me understand the key concepts for my upcoming exam. Please act as a study assistant and be ready to:
1. Explain complex botanical terms in simple, easy-to-understand language.
2. Provide additional examples or real-world analogies for the concepts discussed.
3. Help me formulate potential exam questions or flashcards from this material.
4. Answer any follow-up questions I have about the topics.`

    return `${baseURL}?q=${encodeURIComponent(prompt)}`
}

const menuItems = (pageContent: string) => ({
    markdown: (url: string) => (
    ...
    chatgpt: (url: string) => (
        <a
            href={getPromptUrl("https://chatgpt.com", url, pageContent)}
            target="_blank"
    ...
    claude: (url: string) => (
        <a
            href={getPromptUrl("https://claude.ai/new", url, pageContent)}
            target="_blank"
            rel="noopener noreferrer"
        >
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
...
export function DocsCopyPage({ page, url }: { page: string; url: string }) {
    ...
    const currentMenuItems = menuItems(page)
