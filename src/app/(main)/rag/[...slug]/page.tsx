...
export default async function RagPage({ params }: PageProps) {
    const { slug } = await params;
    const page = await ragSource.getPage(slug);
                ...
                <div className="mx-auto hidden h-24 w-full max-w-3xl items-center gap-4 px-4 sm:flex md:px-0 border-t border-border/40 mt-12 mb-8">
                    {neighbours.prev && (
                        <div className="flex flex-col items-start gap-1">
                            <span className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold px-2">Previous Page</span>
                            <Button
                                variant="outline"
                                size="sm"
                                ...
                                asChild
                            >
                                <Link href={neighbours.prev.url}>
                                    <ArrowLeft className="size-4 mr-2" /> {neighbours.prev.name}
                                </Link>
                            </Button>
                        </div>
                    )}
...
export async function generateMetadata(props: PageProps) {
    ...
    const page = await ragSource.getPage(params.slug);
    ...
    return {
        title: `${page.data.title} | RAG Techniques`,
